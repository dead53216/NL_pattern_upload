...
- **處理樣板直傳**：收集所有明確匹配（tier 0 手動指定吻合 / tier 1 機器類型吻合，含名稱最長機器名）。
  **剛好一個** → 直傳＋聊天欄提示；**多個** → 開面板讓玩家自選（不亂猜）；**零個** → 開面板。
  - **多 match 全可互換視同單一（1.23.0）**：多個 match 的「機器簽章」（有效機器＋電壓＋標籤 icon＋標籤字串）
    全同＝同一台機器掛多個供應器／同款可互換機器 → 挑**剩餘空格最少**者（與面板排序一致、樣板集中）自動直傳；
    簽章有異（真的多台不同機器）才開面板。修「一台機器多供應器永遠開面板、無法自動上傳」。
