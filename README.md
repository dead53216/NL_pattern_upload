...
- **處理樣板直傳**：收集所有明確匹配（tier 0 手動指定吻合 / tier 1 機器類型吻合，含名稱最長機器名）。
  **剛好一個** → 直傳＋聊天欄提示；**多個** → 開面板讓玩家自選（不亂猜）；**零個** → 開面板。
  - **自動直傳嚴格唯一（1.24.0，收回 1.23.0）**：match 多於一個——**同款可互換的兩台機器**——
    一律開面板讓玩家選，不自動直傳。1.23.0 曾以「機器簽章」全同視同單一自動挑一個，
    使用者要求唯一性從嚴（唯一機器、唯一配方，>1 個就不猜），故移除簽章合併（`matchSig` 已刪）。
  - **同機的樣板總成分種類（1.31.0）**：同一台機器可同時掛 **ME 樣板總成**與 **ME 催化劑樣板總成**
    （`MECatalystPatternBufferPartMachine`，催化劑不消耗耐久 → 能做一般總成的事、反之不行）。
    兩者機器身分鍵相同，1.27.0 的合併會誤判為等價而隨便挑一個送。現伺服端加報**容器種類**
    （`containerKind` 尾綴欄位＝容器自身機器物品 id；AE2 供應器為空）：
    **種類不同即不合併**（開面板讓玩家選），排序群組鍵也帶上種類（兩種總成各自成群、不交錯）。
    顯示同步修正——GTO 對樣板總成一律顯示**控制器**機器名，同機兩種總成長得一模一樣，
    現第二行括號改放**容器自身名稱**（「ME催化劑樣板總成」），搜尋鍵一併含它。
  - **同一台機器的多個供應器視同單一（1.27.0）**：多個 match 若**全指向同一台機器且同一模式**，
    送哪個供應器結果都一樣 → 自動直傳（`mergedSingle`），不再為此開面板（一台轉底爐掛兩個供應器即此情形）。
    「同一台」以伺服端新回報的**機器身分鍵**（`machineKey` 尾綴欄位＝多方塊**控制器座標**、單體為機器座標）
    判定——機器類型與物品只證明得了「同款」，兩台同款各掛一個供應器仍照 1.24.0 開面板。
    「同一模式」再比**建議原字串**與**手動指定**：同台機器的供應器可能貼在設不同配方類型的可程式化倉上、
    或被玩家指成不同機器，這種送誰結果不同者不合併。代表取**剩餘空格最少**者（塞快滿的、樣板集中，
    同 1.19.0 排序理念）。身分判不出（舊伺服端／子網歧義／tesseract 混綁）一律不合併，退嚴格唯一。
  - **多 match 診斷（1.26.1）**：開面板時日誌逐筆列出候選——`#index '標籤' via=判定來源 pos=座標鍵
    tier=電壓 machine=機器物品 mkey=機器身分 mode=建議原字串 free=空格`；合併未生效時，靠 `mkey`／`mode`
    一眼判定是「不同台機器」還是「同台不同模式」。
...
- **EMI 上傳改兩階段：等填充落地才編碼（1.35.1，真正的病根）**：EMI 的填充只是把封包送給伺服端，
  客戶端當下看不到結果；而 `gtolib$sendEncodeRequest` 一到伺服端就 `encodePattern()`——
  **填充還沒生效時去編碼，編出來的是上一張樣板**。三個症狀其實同一個根：
  「第一次點沒反應、第二次才成功」（首次編碼失敗 → `gto$currentContainers` 連設都沒設 → 客戶端收不到回覆）、
  「說上傳了實際卻沒有」（送出的是上一張樣板）、「把組裝機的配方丟到分子裝配室」
  （合成判定沒錯，錯的是**內容**——上一張的組裝機樣板被送進分子裝配室）。
  - 現在點下去先**只做填充**，接著每 tick 比對選單快照（模式＋合成格＋`gtocore$recipe`＋處理產物）；
    **一變就代表伺服端已收下並同步回來**，這時才送編碼請求＋清單請求。
    等不到變化（樣板與上一張完全相同 → 快照本來就不會變）就等到上限（20 tick）照送——
    那種情況伺服端狀態本來就是對的。
  - 1.34.1 的「事後等同步」是**時機錯了**：編碼請求早就送出去了，等再久也救不回來，已移除。
  - 連點保護升級：填充等待期間的點擊一併忽略（日誌記一行）。
- **不謊報成功：送出後驗證＋事前問清楚（1.35.0）**：GTOCore 的 `gtolib$sendPattern` 有**七條靜默 return**
  （無 grid／`gto$currentContainers` 為 null／index 越界／`isOutOfService()`／`gto$patternStack` 為 null／
  `gto$canAddPattern` 不過／網路抽不到空白樣板），全部**不回報任何東西**——客戶端過去一律顯示「已上傳」，
  於是出現「跳通知說上傳、實際卻沒有」。
  - **事前**：伺服端隨清單回報 `accept[]` 尾綴欄位＝**照抄 GTO 自己的收下條件**
    （`!isOutOfService() && getTerminalPatternInventory().simulateAdd(本次樣板).isEmpty()`，
    與 `gtolib$sendPattern` 內部同一支）。收不下者 `sortTier` 直接沉底（tier 5）→ **不算明確匹配、
    不自動直傳**；合成直傳也跳過。舊伺服端沒回 → 全 true，行為不變。
  - **事後**：任何單張上傳（自動直傳／合成直傳／面板點列）送出後掛驗證——4 tick 後再問伺服端一次，
    看該目的地是不是**真的有了**這張樣板（`hasRecipe`）。沒有就聊天欄照實說「上傳未生效」，
    日誌另記 `free`／`accept` 供診斷。驗證用的回覆不參與決策與面板刷新。
- **EMI 上傳鈕決策中不吃連點（1.34.3）**：上一次 EMI 上傳還在決策（等清單／等選單同步）時，
  再點只會**重置等待計數** → 連點的話決策永遠跑不完＝按了完全沒反應、也不會有任何提示。
  現在決策中的點擊直接忽略（只記一行日誌）。1.34.2 讓合成／GT 配方都幾乎立即決策後，
  這條只在少數後備情形（非 GT 非合成）才會遇到，但那正是最容易讓人狂點的情形。
- **合成類樣板在 EMI 直接判定（1.34.2 修「合成配方上傳不了分子裝配室」）**：1.34.1 的
  「等選單同步」對合成樣板**永遠等不到**——合成樣板的**處理產物格是空的**，而快照只看
  模式＋`gtocore$recipe`＋處理產物，三者都不變 → 每次都要耗滿 20 次重試才送得出去
  （日誌實測約 2 秒；期間再點一次又重置，於是看起來完全沒反應）。
  現在合成類與 GT 配方一樣**當場定案**：`EmiRecipe.getBackingRecipe()` 丟給 AE2 同一支
  `EncodingHelper.isSupportedCraftingRecipe`（CRAFTING/STONECUTTING/SMITHING），命中即直接走
  `craftDirectSend`（直傳分子裝配室／裝配矩陣），完全不看選單模式。
  順帶：快照補進**合成格內容**（不再只看處理產物），重試也改成每 tick 一次（原本每 2 tick），
  剩下的後備情形（非 GT、非合成）最壞等待從 2 秒降到 1 秒。
...
- **本地重排**（穩定排序，層級）：0 手動指定吻合 → 1 目的地 icon 反查機器支援本類型 **或**
  ...
  - **配方分類對應專屬機器（1.28.0）**：同一配方類型底下可再分 `GTRecipeCategory`——GTO 的
    **魔力組裝**（`mana_assembler`，吃魔力當動力）就是 `ASSEMBLER_RECIPES` 的分類，**魔力組裝機與一般
    組裝機的配方類型完全相同**，型別層級分不出誰該做。現面板同層內先依「分類吻合度」排序：
    魔力組裝樣板 → 魔力組裝機在前、一般組裝機在後；一般組裝樣板 → 反之。分類取
    `GTRecipeDefinition.recipeCategory`（與 currentRecipeType 同趟掃描、同快取；同產物跨分類＝歧義不猜）；
    專屬機器以 registry path 認定（GTO 慣例「分類名＝機器基礎名」，`mana_assembler` ↔ `lv_mana_assembler`…）。
    **只改排序、不動匹配與自動直傳**：兩台仍都算明確匹配（多台照常開面板），網路上只有魔力組裝機時
    一般組裝樣板照樣可手動點選。該類型無專屬分類（絕大多數機器）→ 排序與 1.27.0 完全相同。
  ...
  - **電壓不足列標紅（1.30.0）**：機器類型吻合但電壓跑不動的列，右緣顯示紅字「電壓不足」
    （取代剩餘格數，僅次於「已有該配方」），tooltip 標出**機器電壓 vs 配方需求電壓**。
    這些列本來就在清單裡（1.21.0 起只是沉底、且被自動直傳排除），沒標記時玩家只看到「沒自動上傳」
    卻不知原因。**行為不變**：仍可手動點選上傳。
  - **沒自動直傳一律記錄原因（1.30.0）**：`decidePending` 開面板前必印一行——多個匹配（列出候選）／
    唯一匹配被 extras 押制／類型吻合但全被電壓排除（印配方 tier 與各機器電壓）／無明確匹配／
    **配方類型判不出**（印 `gtocore$recipe` 殘留值與產物簽章）。省去逐一猜測。
...
- **建議機器（接口→存儲總線自動解析）**：接口類供應器 GTOCore 判不出機器。伺服端沿固定拓撲解析——
  ...
  - **樣板總成內建改名救援（1.29.0）**：樣板總成用自身 UI 改名後，GTO 的 `forPatternBuffer` 走
    customName 分支——回「icon＝控制器機器物品、名稱＝**純自訂名**」。機器判定其實還在（icon 反查得到、
    排序照舊 tier 1），但標籤裡沒有機器名了 → 面板只顯示自訂名一行，看起來像「讀不到機器」。
    現補顯示用機器：判準為「icon 反查得到機器**且**標籤不含該機器名」（未改名時 GTO 標籤必含 `%m`），
    命中即比照子網建議路徑顯示**「機器名＋電壓」第一行、「(自訂名)」第二行**，icon 換機器、機器名標青色、
    搜尋鍵含兩者。**純顯示改動**：`Pre.effective`／tier／匹配／自動直傳一律不變。
    與 AE2 供應器改名（1.13.1）互不干擾——那條路 icon 會變成供應器自身物品、反查必為 null，走伺服端建議。
