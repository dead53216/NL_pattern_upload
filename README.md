...
- **建議機器（接口→存儲總線自動解析）**：接口類供應器 GTOCore 判不出機器。伺服端沿固定拓撲解析——
  ...
  - **多類型機器與部件設定（1.18.0）**：建議不再只看機器當下 active 配方類型——
    回報 `getAvailableRecipeTypes()` **全集**（canonical 依 id 排序、逗號串接；濾 DUMMY/HATCH_COMBINED），
    大型冶煉廠（冶煉＋合金冶煉）等多類型機器任一類型的樣板都吻合；客戶端 `pickSuggestion` 擇一顯示
    （優先挑吻合本次樣板者）。部件（輸入倉/總線）若為**可程式化**（gtmthings `IProgrammableMachine`，
    如可程式化倉）且已設定配方類型 → **優先用部件設定**（GTO 自家命名同款優先序）；該介面在 gtocore JiJ
    內嵌、不在編譯 classpath → 反射 `getRecipeType()`（同 IBindable 雷處理）。子網/tesseract 歧義判定
    改以 canonical 建議字串去重——兩台同款機器切不同檔位不會誤判成兩機型。舊客戶端收到逗號串
    tryParse 失敗＝視同無建議（無害降級）；舊伺服端單一 id 新客戶端照收。
  ...
  - **貼「超立方體發生器」追綁定目標（1.14.0）**：GTOCore tesseract（基礎／進階／定向）是**代理機器**——
    ...
    **無配方邏輯**（註冊鏈沒設 recipeType → `getRecipeTypes()` = null）→ GTOCore 標籤顯示 tesseract 自身
    ...
    - **綁定目標取聯集（1.18.1）**：原「唯一機型才建議、多機型歧義回 ""」在 1.18.0 多類型字串下會把
      「多台同款多配方機器、解鎖/配置不同 → 可用類型字串不同」誤判成歧義（1.17.1 單 active 去重時同檔恰好唯一）
      → 讀取不到機器。改取所有綁定機器類型**聯集**：tesseract 本就把 I/O 分派給所有綁定機器，樣板類型吻合
      **任一**綁定機器即可正確上傳；客戶端 `pickSuggestion` 自聯集挑吻合本次樣板者顯示。
