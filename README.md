...
- **處理樣板直傳**：收集所有明確匹配（tier 0 手動指定吻合 / tier 1 機器類型吻合，含名稱最長機器名）。
  **剛好一個** → 直傳＋聊天欄提示；**多個** → 開面板讓玩家自選（不亂猜）；**零個** → 開面板。
  ...
  - **多 match 診斷（1.26.1）**：開面板時日誌逐筆列出候選——`#index '標籤' via=判定來源
    pos=座標鍵 tier=電壓 machine=機器物品 free=空格`（`via` 依 sortTier 同一優先序回推：手動指定／
    伺服端建議／icon 反查／名稱最長機器名）。玩家常只認得其中一台（如「只有一台高爐卻不自動上傳」），
    靠這行才定位得出另一個候選是誰、憑什麼算吻合。
